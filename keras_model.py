...
class Code2VecModel(Code2VecModelBase):
    ...
    def _create_keras_model(self):
        ...
        self.keras_model_predict_function = K.function(
            inputs=inputs, outputs=predict_outputs)
    ...
    def _compile_keras_model(self, optimizer=None):
        ...
        self.keras_eval_model.compile(
            loss={'target_index': 'sparse_categorical_crossentropy',
    ...
    def predict(self, predict_data_rows: Iterable[str]) -> List[ModelPredictionResults]:
        ...
        for input_row in input_iterator:
            ...
            prediction_results = self.keras_model_predict_function(
                input_for_predict)
            ...
            input_row = ReaderInputTensors(
                *common.squeeze_single_batch_dimension_for_np_arrays(input_row))
            ...
            model_prediction_results = ModelPredictionResults(
                original_name=common.binary_to_string(
                    input_row.target_string.item()),
                topk_predicted_words=common.binary_to_string_list(
                    prediction_results.topk_predicted_words),
...
class ModelEvaluationCallback(MultiBatchCallback):
    ...
    def perform_evaluation(self):
        ...
        top_k_acc_formated = ['top{}: {:.4f}'.format(
            i, acc) for i, acc in enumerate(evaluation_results.topk_acc, start=1)]
